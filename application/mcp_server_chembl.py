...
async def compount_activity(compound_name: str) -> List[Dict[str, Any]]:
    ...
    """
    client = new_client
    molecule_id = client.molecule.filter(pref_name__iexact=compound_name).only(
        "molecule_chembl_id"
    )[0]
    # TODO: consider other types of activities
    activity = list(
        client.activity.filter(molecule_chembl_id=molecule_id["molecule_chembl_id"])
...
async def target_activity(target_name: str) -> List[Dict[str, Any]]:
    ...
    """
    client = new_client
    target_id = client.target.filter(
        target_synonym__icontains=target_name, organism="Homo sapiens"
    ).only("target_chembl_id")[0]
    # TODO: consider other types of activities
    activity = list(
        client.activity.filter(target_chembl_id=target_id["target_chembl_id"])
        .filter(standard_type="IC50")
        .only(["pchembl_value", "assay_description", "canonical_smiles"])
    )
    if len(activity) > MAXIMUM_ACTIVITY:
