...
def search_clinical_trials_and_save_studies_to_csv(
    ...
    max_studies: int = 10,
    save_csv: bool = True,
    filename: str = "corona_fields.csv",
    fields: list = None,
) -> str:
    """
    ...
    try:
        ...
        results = ct.get_study_fields(
            search_expr=search_expr, fields=fields, max_studies=max_studies, fmt=fmt
        )
        ...
        if len(results) > 1:  # Header + data
            ...
            if save_csv:
                csv_filename = (
                    filename or f"search_results_{search_expr.replace('+', '_')}.csv"
                )
                df.to_csv(csv_filename, index=False)
                storage_info = (
                    f"Complete results have been saved to file {csv_filename}"
                )
...
def get_studies_by_keyword(
    keyword: str, max_studies: int = 20, save_csv: bool = True, filename: str = None
) -> str:
    """
    ...
    try:
        ...
        if len(results) > 1:  # Header + data
            ...
            if save_csv:
                csv_filename = (
                    filename or f"keyword_results_{keyword.replace(' ', '_')}.csv"
                )
                df.to_csv(csv_filename, index=False)
                storage_info = (
                    f"Complete results have been saved to file {csv_filename}"
