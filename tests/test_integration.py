...
class TestEndToEndWorkflow(unittest.TestCase):
    ...
    def test_complete_workflow_execution(self, mock_create_orchestrator,
                                       mock_clinicaltrials_client, mock_chembl_client,
                                       mock_pubmed_client, mock_arxiv_client, mock_tavily_client):
        """Test complete workflow from query to response"""
        
        # Setup mock clients as context managers
        for client in [mock_tavily_client, mock_arxiv_client, mock_pubmed_client, 
                      mock_chembl_client, mock_clinicaltrials_client]:
        ...
        with patch.object(chat, '_session_manager') as mock_session_manager:
            ...
            result = chat.run_multi_agent_system(self.test_query, "Disable", self.mock_st)
    ...
    @patch('chat.clinicaltrials_mcp_client')
    @patch('chat.create_orchestrator_agent')
    def test_streaming_response_functionality(self, mock_create_orchestrator,
                                            mock_clinicaltrials_client, mock_chembl_client,
                                            mock_pubmed_client, mock_arxiv_client, mock_tavily_client):
        ...
        with patch.object(chat, '_session_manager'):
            result = chat.run_multi_agent_system(self.test_query, "Enable", self.mock_st)
...
class TestErrorHandlingScenarios(unittest.TestCase):
    ...
    @patch('chat.clinicaltrials_mcp_client')
    @patch('chat.create_orchestrator_agent')
    def test_orchestrator_creation_failure(self, mock_create_orchestrator,
                                         mock_clinicaltrials_client, mock_chembl_client,
                                         mock_pubmed_client, mock_arxiv_client, mock_tavily_client):
    ...
    @patch('chat.clinicaltrials_mcp_client')
    @patch('chat.create_orchestrator_agent')
    def test_mcp_client_connection_failure(self, mock_create_orchestrator,
                                         mock_clinicaltrials_client, mock_chembl_client,
                                         mock_pubmed_client, mock_arxiv_client, mock_tavily_client):
        ...
        for client in [mock_arxiv_client, mock_pubmed_client, mock_chembl_client, mock_clinicaltrials_client]:
            client.__enter__ = Mock(return_value=client)
            client.__exit__ = Mock(return_value=None)
    ...
    @patch('chat.clinicaltrials_mcp_client')
    @patch('chat.create_orchestrator_agent')
    def test_orchestrator_streaming_failure(self, mock_create_orchestrator,
                                          mock_clinicaltrials_client, mock_chembl_client,
                                          mock_pubmed_client, mock_arxiv_client, mock_tavily_client):
...
class TestBackwardCompatibility(unittest.TestCase):
    ...
    def test_global_variables_exist(self):
        ...
        for var_name in expected_globals:
            self.assertTrue(hasattr(chat, var_name), 
    ...
    def test_agent_tool_functions_exist(self):
        ...
        for tool_name in expected_tools:
            self.assertTrue(hasattr(chat, tool_name),
                          f"Expected tool function '{tool_name}' not found")
            
            # Verify it's decorated as a tool
            tool_func = getattr(chat, tool_name)
            self.assertTrue(hasattr(tool_func, '__wrapped__') or 
                          hasattr(tool_func, '_strands_tool'),
