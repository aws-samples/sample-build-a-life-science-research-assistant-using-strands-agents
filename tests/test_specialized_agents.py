...
class TestSpecializedAgents(unittest.TestCase):
    ...
    def test_pubmed_research_agent_impl_with_valid_client(self):
        ...
        with patch("chat.get_model") as mock_get_model, patch(
            ...
            call_args = mock_agent_class.call_args
            self.assertIn(
                "pubmed research agent", call_args[1]["system_prompt"].lower()
            )
    ...
    def test_chembl_research_agent_impl_with_valid_client(self):
        ...
        with patch("chat.get_model") as mock_get_model, patch(
            ...
            call_args = mock_agent_class.call_args
            self.assertIn(
                "chembl research agent", call_args[1]["system_prompt"].lower()
            )
    ...
    def test_clinicaltrials_research_agent_impl_with_valid_client(self):
        ...
        with patch("chat.get_model") as mock_get_model, patch(
            ...
            result = chat.clinicaltrials_research_agent_impl(
                self.test_query, self.mock_client
            )
            ...
            call_args = mock_agent_class.call_args
            self.assertIn(
                "clinicaltrials.gov research agent",
                call_args[1]["system_prompt"].lower(),
            )
